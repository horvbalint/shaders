...
float WindTurbine(vec2 uv, float random, float blur) {
  ...
  float deg_60 = 2.*PI/3.;
  float rotation = -(iTime*(random+.5)) + random*60.;
...
vec4 Plane(vec2 uv) {
  ...
  float x_pos = fract(iTime/speed) * width * 2.;
